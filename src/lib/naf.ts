// Mapping complet code NAF rev.2 -> libelle
// Source: INSEE via data.gouv.fr — 732 codes NAF (sous-classe)
// https://www.data.gouv.fr/datasets/codes-naf-insee
...
// Get label for a NAF code, with fallback
export function getNafLabel(code: string | null | undefined): string | null {
  if (!code) return null;
  const normalized = code.trim();
  return NAF_LABELS[normalized] || null;
}

// Format NAF for display: "62.01Z — Programmation informatique"
export function formatNaf(code: string | null | undefined): string | null {
  if (!code) return null;
  const label = getNafLabel(code);
  return label ? `${code} — ${label}` : code;
}

// Get options for filter select, grouped by sector
export const NAF_FILTER_OPTIONS: { value: string; label: string }[] = Object.entries(NAF_LABELS)
