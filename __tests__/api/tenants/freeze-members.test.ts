/**
 * Tests POST /api/tenants/{id}/freeze-members + unfreeze-members — §5.21.
 */
...
function reqFor(path: string) {
  return (raw: string, headers: Record<string, string>) =>
    makeRequest(`/api/tenants/${TENANT_ID}/${path}`, {
      method: "POST",
      headers,
      body: raw,
    });
...
describe("POST /api/tenants/[id]/unfreeze-members", () => {
  beforeEach(() => vi.clearAllMocks());
  const req = reqFor("unfreeze-members");
